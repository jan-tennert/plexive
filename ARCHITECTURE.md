...
backend/
  ...
  tests/edges_test.py           freezes app/graph_edges.py: latent edge -> activation (single UPDATE) -> re-latent on target delete, source delete removes edges, pending casts none + publishing casts them, published->not-published teardown (both sides), read_next cap-3 + latent-marked, person-list featured in / people-connection ignored, clean skip of legacy string refs + missing parts + birth_year-less persons; throwaway SQLite DB via _throwaway_db; run with .venv\Scripts\python.exe tests\edges_test.py
  tests/report_edges_test.py    freezes app/graph_edges.unmatched_latent_edges: mis-keyed (drift) + genuinely-absent latent edges both surface grouped + counted + sorted desc with source title + raw ref, pending-target latent edge excluded, report writes nothing; throwaway SQLite DB via _throwaway_db; run with .venv\Scripts\python.exe tests\report_edges_test.py
  tests/read_next_endpoint_test.py  regression for GET /api/posts/{id}: a published post with featured connections + a featured person returns PostOut.read_next with 3 latent entries (the attach-as-attribute + serialize path); proves "read_next absent" can only be a stale running server, never the committed code; throwaway SQLite DB via _throwaway_db; run with .venv\Scripts\python.exe tests\read_next_endpoint_test.py
...
frontend/
  ...
  src/components/
    ...
    SectionRenderer.tsx         maps section.type → component; handles all sections for books/facts/people/concepts/questions/stories formats; no line dividers (LAYOUT_STANDARD s7: whitespace is the single divider, sections separated by uniform px-6 py-8 rhythm); NO_READ_SECTIONS (at_a_glance/quiz/paper_card/sources) wrapped in data-no-read so read-aloud speaks only prose sections (related_posts case removed — hard cut; a stray one falls to the default warn+skip)
    sections/
