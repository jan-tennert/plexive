...
frontend/
  ...
  src/app/
    ...
    globals.css                 Tailwind import, Lamplight design tokens (@theme: warm surface-0/1/2/3/overlay with widened lightness steps, edge/edge-strong, warm ink levels, lamp/good/bad, fmt-* format inks, radius-card/field/sheet), paper-grain body texture, fixed lamp-light depth gradient (body::after), focus-visible lamp ring, component vocabulary (.card/.btn/.field/.chip/.label-caps/.prose-post; .btn baseline: field radius, sans weight 500, 8/16px default padding; .card carries a matte paper-edge shadow), heart-pop + heart-boom keyframes with reduced-motion guard
