...
backend/
  ...
  app/
    ...
    schemas.py                  Pydantic v2 models: 13 section types with Literal discriminator → AnySection union (related_posts removed as a hard cut — no longer accepted/validated); BooksFeedCard; PostCreate (Books required sections: essence/voices/at_a_glance/heart/core_ideas/takeaway/quiz/sources; image_url recursive check for Supabase storage URL prefix); PostOut (feed_card dict, sections list[dict], tags list[str] + connections list[dict] (graph fields, default []) + read_next list[ReadNextItem] (server-resolved featured edges {target_post_id|None, format, title, latent} from graph_edges.resolved_read_next, populated only by GET /posts/{id}; raw connections kept as-is alongside it), is_user_content bool, like_count int, comment_count int; strips answer_index+explanation from quiz sections so answers never reach the client); PostListOut (PostOut subclass for list endpoints: serializes sections as [] — no list view renders sections, detail page refetches GET /api/posts/{id}); UserOut (incl. avatar_url), InterestOut, EventIn, UploadResponse, SvgUploadResponse
    ...
    routers/
      ...
      feed.py                   GET /api/feed — For You always returns ALL published posts (interests never exclude); tiers (direct match → related co-tag → rest) only order via score_posts, computed in Python from eager-loaded interests; GET /api/feed/following (auth, posts from followed users, limit 50); GET /api/feed/user/{username} (no auth, published posts by user, limit 50); all three respond with PostListOut (sections serialized as [])
      ...
      posts.py                  GET /api/posts/mine (auth, any status); POST /api/posts (auth, 20/day, status="published" if verified else "pending", sets is_user_content=True, computes identity_key via graph_identity.post_identity_key, then rebuilds/activates its graph edges via graph_edges.on_post_written, sanitizes visual_svg fields, interest slugs validated in one IN query); GET /api/posts/{id} (pending visible to author only; attaches resolved read_next via graph_edges.resolved_read_next); _attach_counts() helper adds like_count+comment_count as Python attributes before Pydantic serialization
...
frontend/
  ...
  src/app/
    ...
    globals.css                 Tailwind import, Circuit design tokens (@theme: neutral black surface-0/1/2/3/overlay, neutral gray edge/edge-strong, neutral ink levels, lamp/like/good/bad/save accents, fmt-* format inks, radius-card/field for in-content blocks); neutral dot-grid body texture (body::before, no colored background glow); focus-visible lamp ring; global scrollbar hide both axes (* scrollbar-width:none + *::-webkit-scrollbar display:none, LAYOUT_STANDARD s.5); Stage component vocabulary (.card = borderless frosted slab bg-white/4% + blur, .btn* = borderless pills with springy press scale, .btn-icon = frosted circle, .field = frosted fill with soft radius (single-line inputs add rounded-full at call site), .chip = frosted pill with neutral active fill, .label-caps/.prose-post unchanged); heart-pop + heart-boom + stage-pulse + stage-sheet-in keyframes with reduced-motion guards; ::highlight(read-aloud-sentence/-word) accent washes for read-aloud tracking
    ...
    post/
      [id]/
        page.tsx                full-screen detail page in Stage; floating frosted back circle (btn-icon); read-aloud transport top-right (btn-icon circles: speaker = start, pause/resume in accent ink + stop while reading; useReadAloud over a readable-region wrapper = header + sections, comments excluded, format marker/meta/tag pills marked data-no-read; autostarts via consumeAutoRead when opened from the card speaker; close() stops speech); typographic header per LAYOUT_STANDARD (facts + concepts, gated by the `typographic` predicate) = field line (field label left + small FieldGlyph from card_visual.svg, ~28px, at its right via SvgBlock) then the serif headline once via HeadlineSection, then an optional italic dek (feed_card.one_line, repeated under the headline — concepts carries it, facts has none), then a shared HeaderMeta row mirroring the card footer (round Avatar + @creator left; derived quiz-question count (from the quiz section's array length, not stored in content) + DotScale + reading time pushed right via ml-auto) — format dot+label in the app top bar, the headline section filtered out of the body so it never doubles, tags at the foot of the post (chips near sources); People is a cover format that uses the SAME flat header (coverFlat predicate; flatHeader = typographic || coverFlat gates the top-bar label, end tags and headline filter): role kicker as the field label + the portrait in the glyph slot at its right, name as the single HeadlineSection, lifespan context line, one_line dek, then the shared HeaderMeta — no slab; Books keeps the inset mx-3 slab header (SlabGlow halo + SlabAccent edge, format marker, Books cover/title/author, creator, interest pills); SectionRenderer renders sections in order (body untouched); Read Next block below the readable region (data-no-read, border-t): consumes the server-resolved post.read_next (graph_edges.resolved_read_next), defensively re-capped at 3, rendered via RelatedPostsSection (latent targets show "Coming soon"); no client-side derivation; floating pill comment bar detached left-3/right-3 12px above the safe-area bottom: pill input + circular arrow-up submit + bare like heart circle, no count (liked bg-like/10; save/share live on the feed rail, not here); loading = stage-pulse slabs, not-found = slab message; slide-up animation, swipe-right-to-close; creator = round Avatar + @username from the same author fields as the card footer (no hardcoded Deepscroll fallback); comment add/delete writes the new count through to the cached feed lists
    components/
  ...
  src/components/
    ...
    DotScale.tsx                shared three-dot difficulty scale (1-3 filled neutral dots); used by the feed card footer (PostCard) and the facts detail meta row so both render identically
    SectionLabel.tsx            unified section header (h3, label-caps) rendered in the format accent, identical on every section per LAYOUT_STANDARD s7 (the repeated accent caps label is the through-line; only --accent differs per format); data-no-read keeps labels out of read-aloud
    ...
    sections/
      ...
      FormalDefinitionSection.tsx concepts: body + KaTeX display formula + notation legend (symbols rendered via MathText); same math kit as FormalismSection
...
mobile/                         React Native app (Expo SDK 56, TypeScript, expo-router, NativeWind 4 + tailwindcss 3.4); phase 5 = profiles, follows, search and stats in Stage; chat = live DMs and groups over the websocket
  ...
  src/components/SectionRenderer.tsx  port of web SectionRenderer: sorts by order, dispatches all section types, isUserContent to SVG sections, postId to QuizSection; divide-y becomes per-section borderTop; related_posts case removed (hard cut — a stray one falls to the default warn+skip)
  src/components/sections/      80 section ports mirroring frontend sections/ 1:1; primitives.tsx supplies SectionBlock (px-6 py-8), SectionLabel (.label-caps), Prose (.prose-post 17/29 serif), sans/mono style builders, SvgFigure (viewBox aspect parse + SafeSvg + max-width), CaptionedImage (resolveImageUrl), makeLabeledProse factory (the ~30 label+paragraph sections are one-liners), NumberBubble; RelatedPostsSection no longer a dispatched section — it renders the detail screen's server read_next (ReadNextItem[], resolved cards clickable, latent dimmed "Coming soon")
...
| sections/RelatedPostsSection.tsx | horizontal scroll row; renders the server read-next set (ReadNextItem[], used only by the detail page's Read Next block); target_post_id null (latent) → non-clickable with "Coming soon" label |
...
| sections/HeadlineSection.tsx | large left-aligned serif headline (font-medium tracking-tight leading-snug, max-w-[24ch] measure) matching the feed card headline voice (LAYOUT_STANDARD s.6, shares left axis with field line/meta/body); digit runs + optional scale words highlighted in cyan-400 via regex |
