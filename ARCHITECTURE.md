...
backend/
  ...
  app/
    ...
    routers/
      ...
      posts.py                  GET /api/posts/mine (auth, any status); POST /api/posts (auth, 20/day, status="published" if verified else "pending", sets is_user_content=True, computes identity_key via graph_identity.post_identity_key, sanitizes visual_svg fields, interest slugs validated in one IN query); GET /api/posts/{id} (pending visible to author only); _attach_counts() helper adds like_count+comment_count as Python attributes before Pydantic serialization
