...
frontend/
  ...
  src/components/
    ...
    SectionLabel.tsx            unified section header (h3, label-caps) rendered in the format accent with a short accent tick (h-px w-4 rule) before the text, identical on every section per LAYOUT_STANDARD s7 (only --accent differs per format); data-no-read keeps labels out of read-aloud
    ...
    SectionRenderer.tsx         maps section.type → component; handles all sections for books/facts/people/concepts/questions/stories formats; no line dividers (LAYOUT_STANDARD s7: whitespace is the single divider, sections separated by uniform px-6 py-8 rhythm); NO_READ_SECTIONS (at_a_glance/quiz/quiz_badge/paper_card/related_posts/sources) wrapped in data-no-read so read-aloud speaks only prose sections
    sections/
...
| sections/SurprisesSection.tsx | "Why It Surprises Us" label; the single tinted key-section box for facts (bg-(--accent)/20, LAYOUT_STANDARD s7); prose in zinc-200 |
